(() => {
  ...
  function resetManifestQuestion() {
    manifestQuestion = 0;
    const mqItems = document.querySelectorAll('.mq-item');
    mqItems.forEach(item => item.classList.toggle('active', item.dataset.q === '0'));
    const mqLabel = document.getElementById('manifestQuestionLabel');
    if (mqLabel) mqLabel.textContent = 'Are you a Pirate, or are you a Paragon?';
  ...
  async function boot(event) {
    ...
    function activateEnergyMode({ showInspector = true } = {}) {
      energyMode = true;
      healthMode = false;
      els.tooltip.style.display = 'none';
      ...
      if (showInspector) showPanel('energy', selectedEnergySystem || focusedEnergySystem);
      if (els.openFiltersBtn) els.openFiltersBtn.classList.remove('visible');
      closeCrewRoll();
    }

    function deactivateEnergyMode() {
      energyMode = false;
      resetEnergyElevation();
      closePanel();
    }

    function activateManifestMode({ openCrew = APP_CONFIG.autoOpenManifestCrew } = {}) {
      healthMode = true;
      energyMode = false;
      resetManifestQuestion();
      resetEnergyElevation();
    ...
    function deactivateManifestMode() {
      healthMode = false;
      els.tooltip.style.display = 'none';
      closePanel();
      closeCrewRoll();
    ...
    els.energyBtn.addEventListener('click', () => {
      if (energyMode) deactivateEnergyMode();
      else activateEnergyMode({ showInspector: true });
      updateButtons();
    });
    ...
    els.healthBtn.addEventListener('click', () => {
      if (healthMode) deactivateManifestMode();
      else activateManifestMode({ openCrew: APP_CONFIG.autoOpenManifestCrew });
      updateButtons();
    });
    Object.assign(window.EarthHealthEnergyApp, {
      setMode(mode, options = {}) {
        if (mode === 'energy') {
          activateEnergyMode({ showInspector: options.showInspector !== false });
        } else if (mode === 'manifest') {
          activateManifestMode({ openCrew: options.openCrew ?? APP_CONFIG.autoOpenManifestCrew });
        } else {
          if (energyMode) deactivateEnergyMode();
          if (healthMode) deactivateManifestMode();
        }
    ...
    if (APP_CONFIG.initialMode === 'energy' || APP_CONFIG.mode === 'energy') {
      activateEnergyMode({ showInspector: APP_CONFIG.showInitialPanel !== false });
    } else if (APP_CONFIG.initialMode === 'manifest' || APP_CONFIG.mode === 'manifest') {
      activateManifestMode({ openCrew: APP_CONFIG.autoOpenManifestCrew });
    }
    updateButtons();
